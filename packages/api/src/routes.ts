import { AppRouteDefs } from '../typings/app';
import { addJobHandler } from './handlers/addJob';
import { cleanAllHandler } from './handlers/cleanAll';
import { cleanJobHandler } from './handlers/cleanJob';
import { emptyQueueHandler } from './handlers/emptyQueue';
import { entryPoint } from './handlers/entryPoint';
import { jobLogsHandler } from './handlers/jobLogs';
import { jobHandler } from './handlers/job';
import { pauseQueueHandler } from './handlers/pauseQueue';
import { promoteJobHandler } from './handlers/promotJob';
import { queuesHandler } from './handlers/queues';
import { redisStatsHandler } from './handlers/redisStats';
import { resumeQueueHandler } from './handlers/resumeQueue';
import { retryAllHandler } from './handlers/retryAll';
import { retryJobHandler } from './handlers/retryJob';
import { promoteAllHandler } from './handlers/promoteAll';
import { updateJobDataHandler } from './handlers/updateJobData';
import { pauseAllHandler } from './handlers/pauseAll';
import { resumeAllHandler } from './handlers/resumeAll';
import {
  createConnectionHandler,
  listConnectionsHandler,
  getConnectionHealthHandler,
  deleteConnectionHandler,
  addQueueToConnectionHandler,
  getConnectionQueuesHandler,
  removeQueueFromConnectionHandler,
} from './handlers/connections';
import {
  detectQueuesHandler,
  detectConnectionQueuesHandler,
} from './handlers/queueDetection';

export const appRoutes: AppRouteDefs = {
  entryPoint: {
    method: 'get',
    route: ['/', '/queue/:queueName', '/queue/:queueName/:jobId'],
    handler: entryPoint,
  },
  api: [
    { method: 'get', route: '/api/redis/stats', handler: redisStatsHandler },
    { method: 'get', route: '/api/queues', handler: queuesHandler },
    // Queue detection routes
    { method: 'post', route: '/api/queues/detect', handler: detectQueuesHandler },
    // Dynamic connection management routes
    { method: 'post', route: '/api/connections', handler: createConnectionHandler },
    { method: 'get', route: '/api/connections', handler: listConnectionsHandler },
    { method: 'get', route: '/api/connections/:id/health', handler: getConnectionHealthHandler },
    { method: 'delete', route: '/api/connections/:id', handler: deleteConnectionHandler },
    { method: 'post', route: '/api/connections/:id/queues', handler: addQueueToConnectionHandler },
    { method: 'get', route: '/api/connections/:id/queues', handler: getConnectionQueuesHandler },
    { method: 'delete', route: '/api/connections/:id/queues/:queueName', handler: removeQueueFromConnectionHandler },
    { method: 'post', route: '/api/connections/:id/detect', handler: detectConnectionQueuesHandler },
    { method: 'put', route: '/api/queues/pause', handler: pauseAllHandler },
    { method: 'put', route: '/api/queues/resume', handler: resumeAllHandler },
    {
      method: 'get',
      route: '/api/queues/:connectionId/:queueName/:jobId/logs',
      handler: jobLogsHandler,
    },
    {
      method: 'get',
      route: '/api/queues/:connectionId/:queueName/:jobId',
      handler: jobHandler,
    },
    {
      method: 'post',
      route: '/api/queues/:connectionId/:queueName/add',
      handler: addJobHandler,
    },
    {
      method: 'put',
      route: '/api/queues/:connectionId/:queueName/retry/:queueStatus',
      handler: retryAllHandler,
    },
    {
      method: 'put',
      route: '/api/queues/:connectionId/:queueName/promote',
      handler: promoteAllHandler,
    },
    {
      method: 'put',
      route: '/api/queues/:connectionId/:queueName/clean/:queueStatus',
      handler: cleanAllHandler,
    },
    {
      method: 'put',
      route: '/api/queues/:connectionId/:queueName/pause',
      handler: pauseQueueHandler,
    },
    {
      method: 'put',
      route: '/api/queues/:connectionId/:queueName/resume',
      handler: resumeQueueHandler,
    },
    {
      method: 'put',
      route: '/api/queues/:connectionId/:queueName/empty',
      handler: emptyQueueHandler,
    },
    {
      method: 'put',
      route: '/api/queues/:connectionId/:queueName/:jobId/retry/:queueStatus',
      handler: retryJobHandler,
    },
    {
      method: 'put',
      route: '/api/queues/:connectionId/:queueName/:jobId/clean',
      handler: cleanJobHandler,
    },
    {
      method: 'put',
      route: '/api/queues/:connectionId/:queueName/:jobId/promote',
      handler: promoteJobHandler,
    },
    {
      method: 'patch',
      route: '/api/queues/:connectionId/:queueName/:jobId/update-data',
      handler: updateJobDataHandler,
    },
  ],
};
